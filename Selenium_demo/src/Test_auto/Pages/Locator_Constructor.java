...
//		Phone.sendKeys(String.valueOf(phone_num));
	...
	public void ScrollToTable() {
		...
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", tb);
		try {
			Thread.sleep(2000);
		        ...
		        if (cells.size() >= 4) {
		            ...
		            if (userRole.equalsIgnoreCase("ESS")) {
		            	if(!checkbox.isSelected()) {
		            		...
		            		checkbox.click();
//		            		Assert.assertEquals(username, "Ngan");
		            	}	
		            	
		                System.out.println("Username with ESS role: " + username);  
		                Assert.assertTrue(checkbox.isSelected(), "❌ Checkbox should be selected for ESS user!");
		                countChecked++;
		            }
		        }
		    }	
			Assert.assertTrue(countChecked > 0, "❌ No ESS users were checked!");
		    System.out.println("✅ All ESS users were successfully checked: " + countChecked);
